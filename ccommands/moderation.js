...
function createPremiumEmbed(title, executioner) {
    return new EmbedBuilder()
        .setTitle(`🔹 Nexora Matrix | ${title}`)
        .setColor(0x1A1C1E) // Fondo oscuro ultra premium estilo Nexora
        .setThumbnail('https://i.imgur.com/wSTFk9g.png') // Puedes cambiar esto por el logo de tu bot
        .setFooter({ 
            text: `Action Authorized by ${executioner.username}`, 
...
module.exports = [
    ...
    // 26. ROLE DELETE
    {
        ...
        permissions: [PermissionFlagsBits.ManageRoles],
        options: [{ name: 'role', description: 'Target role removal key reference mapping', type: 8, required: true }],
        async run(context, args) {
            const targetRoleAsset = parseTargetRole(context, context.isCommand?.() ? null : args);
            if (!targetRoleAsset) return context.reply({ content: '❌ System could not map reference match key to any existing server roles.', ephemeral: true });
            ...
            const execUser = context.isCommand?.() ? context.user : context.author;
            const embed = createPremiumEmbed('Security Role Purged', execUser)
                .addFields({ name: '🗑️ Data Signature Unlinked', value: `> **Role Name:** \`${savedRoleName}\`\n> **ID Removed:** \`${savedRoleID}\``, inline: false });
