...
{
    name: 'roledel',
    description: 'Deletes a custom server role configuration asset.',
    permissions: [PermissionFlagsBits.ManageRoles],

    options: [
        {
            name: 'role',
            description: 'Target role removal key reference mapping',
            type: 8,
            required: true
        }
    ],
    ...
    async run(context, args) {
        ...
        const targetRoleAsset = parseTargetRole(
            context,
            isSlash ? null : args
        );
        ...
        if (!targetRoleAsset) {
            if (isSlash) {
                return context.reply({
                    content: '❌ System could not map reference match key to any existing server roles.',
                    ephemeral: true
                });
            }

            return context.reply(
                '❌ System could not map reference match key to any existing server roles.'
            );
        }

        const savedRoleName = targetRoleAsset.name;
        const savedRoleID = targetRoleAsset.id;

        await targetRoleAsset.delete();

        const execUser = isSlash ? context.user : context.author;

        const embed = createPremiumEmbed(
            'Security Role Purged',
            `🗑️ **Data Signature Unlinked**\n\n> **Role Name:** \`${savedRoleName}\`
> **ID Removed:** \`${savedRoleID}\``,
            execUser
        );
...
{
    name: 'avatar',
    description: 'Displays the target user avatar image with download links.',
    ...
    async run(context, args) {
        ...
        const target =
            await parseTargetUser(context, args) ||
        ...
        const avatarJpeg = target.displayAvatarURL({
            extension: 'jpeg',
            size: 2048
        });
        ...
        const embed = createPremiumEmbed(
            'User Avatar Profile',
            `✨ **Full resolution image render for** ${target}
...
> **Roles:** \`${totalRoles}\` roles`,
                ...
                },
                {
                    name: '📡 Channel Layout',
                    value:
                        `> **Text Nodes:** \`${textChannels}\`
> **Voice Streams:** \`${voiceChannels}\``,
                    inline: true
                },
                {
                    name: '🚀 Server Power Boosts',
                    value:
                        `> **Boost Count:** \`${boostCount}\`
> **Tier Level:** \`Tier ${guild.premiumTier}\``,
                    inline: false
                }
            );
...
// 2.1 SERVER INFO ALIAS
{
    ...
    async run(context) {
        const serverinfoCommand = module.exports.find(
            cmd => cmd.name === 'serverinfo'
        );
...
// 3. USER INFO
{
    ...
    options: [
        {
            name: 'user',
            description: 'The member whose profile stats you want to view',
            type: 6,
            required: false
        }
    ],
    ...
    async run(context, args) {
        ...
        if (!member) {
            if (isSlash) {
                return context.reply({
                    content: '❌ Lookup Error: Profile does not exist inside this server guild context.',
                    ephemeral: true
                });
            }

            return context.reply(
                '❌ Lookup Error: Profile does not exist inside this server guild context.'
            );
        }
...
> **ID:** \`${target.id}\``,
                ...
                },
                {
                    ...
                    value:
                        `> **Created Account:** <t:${createdTimestamp}:R> (<t:${createdTimestamp}:f>)
> **Joined Server:** <t:${joinedTimestamp}:R> (<t:${joinedTimestamp}:f>)`,
                    inline: false
                },
                {
                    name: '🛡️ Local Guild Privileges',
                    value:
...
// 3.1 USER INFO ALIAS
{
    ...
    async run(context, args) {
        const userinfoCommand = module.exports.find(
            cmd => cmd.name === 'userinfo'
        );
...
// 4. SAY
{
    ...
    options: [
        {
            name: 'message',
            description: 'The text message you want the bot to say',
            type: 3,
            required: true
        }
    ],
    ...
    async run(context, args) {
        ...
        if (!rawMessage) {
            if (isSlash) {
                return context.reply({
                    content: '❌ Input Error: Please specify a text message sequence.',
                    ephemeral: true
                });
            }

            return context.reply(
                '❌ Input Error: Please specify a text message sequence.'
            );
        }
        ...
        return isSlash
            ? context.reply({ content: rawMessage })
            : context.channel.send({ content: rawMessage });
    }
},
...
{
    name: 'embed',
    description: 'Sends a custom stylized embed card.',
    ...
    options: [
        ...
        {
            name: 'title',
            description: 'The title header of the card',
            type: 3,
            required: true
        },
        {
            name: 'description',
            description: 'The main body text of the card',
            type: 3,
            required: true
        }
    ],
    ...
    async run(context, args) {
        ...
        if (!colorHex || !title || !description) {
            ...
            return context.reply(errorHelp);
        }

        let resolvedColor = parseInt(
            colorHex.replace('#', ''),
            16
        );

        if (isNaN(resolvedColor)) {
            resolvedColor = 0x1A1C1E;
        }
        ...
        return isSlash
            ? context.reply({ embeds: [customEmbed] })
            : context.channel.send({ embeds: [customEmbed] });
    }
}
